...
void Player::Update(float dt)
{
	...
	if (INPUT.GetKeyDown(SDL_SCANCODE_LEFT)) m_transform.rotation -= Math::DegToRad(120) * dt;
	if (INPUT.GetKeyDown(SDL_SCANCODE_RIGHT)) m_transform.rotation += Math::DegToRad(120) * dt;

	m_velocity += Vector2{ thrust, 0.0f }.Rotate(m_transform.rotation);
	m_velocity.x = Math::Clamp(m_velocity.x, -1000.0f, 1000.0f);
